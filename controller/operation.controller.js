...
const { LATE_LOGIN } = require('../helpers/constant');
...
module.exports = {
    lateLogin: async (req, res) => {
      ...
      const isLateLoginResp = await client.get(config.REDIS_PREFIX+LATE_LOGIN + req.staffId);
      if(isLateLoginResp) return res.json({message:'You have already logged in'});         
        const {data:lateLoginResp} = await operationService.checkLateLogin(
          ...
          await client.set(
            config.REDIS_PREFIX+ LATE_LOGIN + req.staffId,
